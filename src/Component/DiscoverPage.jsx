    ...
    return(
        ...
        <div className="container-fluid">
            ...
            <div className="flex justify-between text-white mt-8 px-5">
                ...
                <a href="../App.js" className="underline underline-offset-2 mt-3">see all</a>
            </div>
            <Slider/>
                ...
                {images.map((image,idx)=>{
                    return(
                        ...
                        <a href={image.url} key={image.id}>
                            <img src={image.images.jpg.image_url} className="my-4 h-36 w-24 sm:h-48 sm:w-36 xl:h-60 xl:w-48 " alt={image.id}/>
                        </a>
                        
                        <div className="flex items-center justify-center">
                            <div className="grid grid-cols-1">        
                                    <h4 className="text-white font-bold px-3 text-mobile text-left "key={image.id}>{image.title}</h4>
                                    <div className="desc grid grid-rows-1 gap-y-1 md:gap-y-1  pl-3 text-left ">
                                            <p className="text-slate-400 text-eps  "key={image.id}>Eps: {image.episodes} - {image.duration} </p>
                                            <p className="text-slate-400 text-eps w-16 md:w-full "key={image.id}>Status: {image.status} - {image.duration}</p>
                                            <div className="grid grid-cols-2 text-white  md:w-full">
                                                <div className="rate flex items-center ">
                                                    <span className="inline-block align-middle pr-1">
                                                        <img src="asset/star.png" className="w-2 h-2 md:w-4 md:h-4" alt="star"/>
                                                    </span>
                                                    <p className="text-white text-eps"key={image.id}>{image.score}</p>
                                                </div>
                                                <p className="rating flex md:justify-center items-center text-eps"key={image.id}>{image.type}</p>
                                        </div>
                                    </div>
